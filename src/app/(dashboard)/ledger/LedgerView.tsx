...
export function LedgerView() {
  ...
  const filtered = search
    ? customers.filter((c) =>
        [c.name, c.phone, c.city].join(" ").toLowerCase().includes(search.toLowerCase())
      )
    : customers;

  return (
    <div>
      <h2 className="font-serif text-lg mb-4">Ledger</h2>
      ...
      <div className="grid gap-4 grid-cols-2 max-lg:grid-cols-1 mb-4">
        <div className="bg-white dark:bg-[#1c1917] border border-[#e6e0d4] dark:border-[#2e2a25] rounded-xl p-5">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-sm font-semibold">Cash Flow (Cash Book)</h3>
            <Link href="/cashbook" className="text-[10px] text-amber-700 dark:text-amber-400 hover:underline">View all →</Link>
          </div>
          ...
          {recentCashEntries && recentCashEntries.length > 0 && (
            <div className="mt-3 pt-3 border-t border-[#e6e0d4] dark:border-[#2e2a25]">
              ...
              {recentCashEntries.slice(0, 3).map((e: any) => (
        ...
        <div className="bg-white dark:bg-[#1c1917] border border-[#e6e0d4] dark:border-[#2e2a25] rounded-xl p-5">
          ...
          {recentExpenses && recentExpenses.length > 0 && (
            <div className="pt-3 border-t border-[#e6e0d4] dark:border-[#2e2a25]">
              ...
              {recentExpenses.slice(0, 3).map((e: any) => (
                <div key={e.id} className="flex justify-between text-xs py-1">
              ...
              <tbody>
                {filtered.map((c) => (
                  <tr key={c.id} className="border-b border-[#ede8dc] dark:border-[#2e2a25] hover:bg-amber-50/50 dark:hover:bg-amber-950/20">
                    <td className="p-3">
                      <Link href={`/customers/${c.id}`} className="font-semibold hover:text-amber-700 dark:hover:text-amber-400 hover:underline">
                        {c.name}
                      </Link>
                    ...
                    <td className={`p-3 text-right font-semibold ${c.outstanding > 0 ? "text-red-700 dark:text-red-400" : "text-[#504d44] dark:text-[#d6cfc5]"}`}>
                    ...
                    <td className="p-3 text-right">
                      <Link href={`/customers/${c.id}`} className="text-xs text-amber-700 dark:text-amber-400 hover:underline">View →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
