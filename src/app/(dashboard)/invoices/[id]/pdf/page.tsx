    ...
    <div className="print-container bg-white text-black p-8 max-w-[800px] mx-auto" style={{ fontFamily: "Arial, sans-serif" }}>
      ...
      <div className="flex justify-between items-start border-b-2 border-amber-600 pb-4 mb-6">
        ...
        <div className="text-right">
          <div className="text-lg font-bold text-amber-700">TAX INVOICE</div>
          <div className="text-xs text-gray-700 font-mono">{inv.invoiceNo}</div>
          <div className="text-xs text-gray-700 mt-1">Date: {formatDate(inv.invoiceDate)}</div>
          {inv.dueDate && <div className="text-xs text-gray-700">Due: {formatDate(inv.dueDate)}</div>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          ...
          {inv.description && <div className="text-xs text-gray-700 mt-1">{inv.description}</div>}
        </div>
        <div className="text-right">
          <div className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold mb-1">Status</div>
          <div className="text-xs uppercase font-semibold text-amber-700">{inv.status}</div>
        </div>
      </div>
      ...
      <table className="w-full text-sm border border-gray-300 mb-4" style={{ borderCollapse: "collapse" }}>
        <thead>
          <tr className="bg-gray-100 border-b border-gray-300 text-[10px] uppercase tracking-wider">
            ...
            <th className="text-right p-2 border-r border-gray-300">GST%</th>
          ...
          {inv.items.length === 0 ? (
            ...
            inv.items.map((i, idx) => (
              <tr key={i.id} className="border-b border-gray-200">
                ...
                <td className="p-2 border-r border-gray-200 text-right text-xs">{i.gstPercentage}%</td>
      ...
      <div className="flex justify-end mb-8">
        <div className="w-64 text-sm">
          ...
          <div className="flex justify-between py-1 text-gray-700"><span>GST</span><span>{formatINR(inv.gstTotal)}</span></div>
      ...
      {/* Amount in words */}
      <div className="text-xs text-gray-700 mb-6 italic">
        Amount in words: {numberToWords(Math.round(inv.grandTotal))} Rupees Only.
      </div>
      ...
      <div className="text-xs text-gray-600 mb-8">
        ...
        <ol className="list-decimal list-inside space-y-0.5">
          <li>Payment due within 15 days of invoice date unless otherwise agreed.</li>
          <li>Goods once sold are not returnable. Warranty per manufacturer terms.</li>
      ...
      <div className="flex justify-between items-end mt-12">
        <div className="text-xs text-gray-600">
          <div>For any queries: +91 98765 43210</div>
          <div>Email: contact@karshani.example</div>
        </div>
        <div className="text-center">
          <div className="border-t border-gray-400 pt-1 w-48 text-xs text-gray-700">For Karshani Enterprises</div>
        </div>
      </div>
